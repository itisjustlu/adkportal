...
function template_adk_post_install_below(){}
function template_adk_post_install_above(){}

function template_adk_post_install(){

	global $txt, $modSettings, $scripturl, $contex, $adkFolder;
	
	echo'
	<div class="cat_bar">
		<h4 class="catbg">
			',$txt['adk_post_install'],'
		</h4>
	</div>';
	...
	$tables_created = array(
		...
		'adk_blocks_template_admin', 'adk_pages_comments'
	);

	$columns_added = array(
		'adk_notes' => 'members',
		'adk_pages_notifications' => 'members',
		'id_new' => 'topics',
	);
	...
	<span class="clear upperframe">
		...
		<div class="roundframe">
			<img class="adk_vertical_align" alt="" src="'.$adkFolder['images'].'/accept.png" />&nbsp;',$txt['adk_post_install_2'];

	foreach($tables_created AS $table_name)
		echo'<br />&nbsp;&nbsp;<img class="adk_vertical_align" alt="" src="'.$adkFolder['images'].'/bullet_red.png" />',$table_name;


	echo'
		<br /><img class="adk_vertical_align" alt="" src="'.$adkFolder['images'].'/accept.png" />&nbsp;',$txt['adk_post_install_3'];

	foreach($columns_added AS $column => $table){

		$replace = $txt['adk_post_install_3'];
		$replace = str_replace('{$name_column}', $column, $replace);
		$replace = str_replace('{$name_table}', 'smf_'.$table, $replace);

		echo'<br />&nbsp;&nbsp;<img class="adk_vertical_align" alt="" src="'.$adkFolder['images'].'/bullet_red.png" />',$replace;
	}
